...
try:
    ...
    print("missing pytorch3d — using a pure-torch farthest-point-sampling fallback "
          "(no custom CUDA extension required, functionally equivalent)")

    def fps(points, num_points=1024, use_cuda=True):
        """Greedy farthest-point sampling, pure torch (no pytorch3d/CUDA-extension
        dependency). Same call signature/return contract as the pytorch3d-backed
        version above: sampled_points (N, 3) array-like, indices a torch tensor
        shaped (1, num_points) (callers do `index.detach().cpu().numpy()[0]`)."""
        ...
        if k < num_points:
            # caller (pad_array) guarantees n >= num_points before calling fps(),
            # but guard anyway: pad by resampling with replacement.
            pad = indices[torch.randint(0, k, (num_points - k,), device=device)]
            indices = torch.cat([indices, pad])

        sampled_points = pts[indices].cpu().numpy()
